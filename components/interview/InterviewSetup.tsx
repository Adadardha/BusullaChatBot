  ...
  return (
    ...
    >
      <div className="brutalist-border bg-black p-6 md:p-8 lg:p-12">
        ...
        {/* Career Info */}
        <div className="mb-8 p-4 md:p-6 brutalist-border bg-white/5">
                ...
                >
                  ...
                  {isSelected && (
                    <motion.div
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="mt-3 text-xs font-bold uppercase text-white"
                    >
                      ✓ E ZGJEDHUR
                    </motion.div>
                  )}
            ...
            {difficulties.map((difficulty) => {
              ...
              return (
                <motion.button
                  ...
                  className={`p-4 md:p-6 text-left transition-all ${
                    isSelected
                      ? `brutalist-border ${info.bgColor}`
                      : 'border-2 border-white/20 hover:border-white/40'
                  }`}
                  whileHover={{ scale: isSelected ? 1 : 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  <p className={`font-bold text-base md:text-lg ${info.color}`}>
                    {info.name}
                  </p>
                  <p className="text-xs md:text-sm text-gray-400 mt-1">
                    {info.description}
